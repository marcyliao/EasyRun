...
public class ActivityRecordActivity extends Activity {
	...
	private void prepareWidgets() {
		...
		((TextView) findViewById(R.id.TextViewAVGSpeed)).setText(fp
				.getSpeed(record.getAvgSpeed()));
		((TextView) findViewById(R.id.TextViewTime)).setText(fp
				.getDuration(record.getTimeLength()));
		((TextView) findViewById(R.id.TextViewCalories)).setText(fp
				.getCalories(record.getCalories()));
		((TextView) findViewById(R.id.TextViewDistance)).setText(fp
				.getDistance(record.getDistance()));

		if (record.getNote() != null && !record.getNote().equals(""))
			((TextView) findViewById(R.id.TextViewNote)).setText(record
					.getNote());
	...
	private void prepareDistanceUnitWidget(Context context) {
		...
		((TextView) findViewById(R.id.textViewDistanceUnit)).setText(fp
				.getDistanceUnit());
		((TextView) findViewById(R.id.textViewSpeedUnit)).setText(fp
				.getSpeedUnit());
	}
	...
	private void prepareMap() {
		...
		if (record.getLocationPoints().size() >= 1) {
			...
			ActivityRecord.Location startLocation = record.getLocationPoints()
					.get(0);
			map.addMarker(new MarkerOptions().position(
					getLatLngFromLocation(startLocation)).icon(
					BitmapDescriptorFactory
							.defaultMarker(BitmapDescriptorFactory.HUE_RED)));

			// Add the end marker
			int last = record.getLocationPoints().size() - 1;
			ActivityRecord.Location lastLocation = record.getLocationPoints()
					.get(last);
			map.addMarker(new MarkerOptions().position(
					getLatLngFromLocation(lastLocation)).icon(
					BitmapDescriptorFactory
							.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)));

			// Move the camera instantly to hamburg with a zoom of 15.
			map.moveCamera(CameraUpdateFactory.newLatLngZoom(
					getLatLngFromLocation(lastLocation), 15));
		}
		...
		if (record.getLocationPoints().size() > 1) {
			ActivityRecord.Location originalLocation = record
			...
			for (int i = 1; i < record.getLocationPoints().size(); i++) {
				...
				ActivityRecord.Location endLocation = record
						.getLocationPoints().get(i);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_share:
			...
			new ShareUtility().share(this, content,
					findViewById(R.id.activityRecordWindow), map);
